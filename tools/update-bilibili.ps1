...
$arguments = @(
  ...
  (ConvertFrom-Base64Utf8 "ZmlsdGVyVmVydGljYWxWaWRlbz1zd2l0Y2gsIGZhbHNlLCB0cnVlLCB0YWc96L+H5ruk44CM56uW5bGP6KeG6aKR44CNLCBkZXNjPS0gdHJ1ZTog5byA5ZCvXG4tIGZhbHNlOiDlhbPpl60="),
...
$popularHotFilterTag = ConvertFrom-Base64Utf8 "6L+H5ruk54Ot5pCc54Ot6Zeo6K+d6aKY"
$popularHotFilterScript = "http-response ^https:\/\/(?:app\.bilibili\.com|grpc\.biliapi\.net)\/bilibili\.app\.show\.v1\.Popular\/Index$ script-path=https://rucu6.pages.dev/Scripts/bilibili/response.js, requires-body=true, binary-body-mode=true, tag=$popularHotFilterTag, enable={filterHotSearch}"
...
$feedIndexTag = ConvertFrom-Base64Utf8 "6L+H5ruk6aaW6aG15o6o6I2Q5rWB"
$feedIndexScript = "http-response ^https:\/\/app\.bilibili\.com\/x\/v2\/feed\/index script-path=$repoRawBase/Scripts/bilibili/feed-index.js, requires-body=true, tag=$feedIndexTag, argument=[{customizeHomeFeed},{filterVerticalVideo}]"
...
for ($i = 0; $i -lt $lines.Count; $i++) {
  ...
  if ($line -like '*app\.bilibili\.com\/x\/v2\/feed\/index*') {
    $newLines.Add($feedIndexScript)
    continue
  }

  if ($line -like '*bilibili\.app\.show\.v1\.Popular\/Index*') {
    $newLines.Add($popularHotFilterScript)
